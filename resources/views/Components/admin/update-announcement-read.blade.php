<x-admin-layout>
    ...
    {{-- Update Form --}}
    <form id="update-form" method="POST" action="{{ route('admin.announcements.update', $announcement->id) }}" enctype="multipart/form-data" class="flex flex-col gap-5 rounded-xl bg-white/10 backdrop-blur-lg shadow-md p-6 border border-gray-200">
        @csrf
        @method('PUT')
        ...
        <div class="flex items-center justify-center">
            <div class="w-lg aspect-[3/2] relative group flex flex-col gap-2 text-sm">
                <label class="font-bold text-lg" for="image">Announcement Image</label>
                <input type="file" name="image" accept="image/*" class="bg-[#F1EAEA] p-2 rounded-lg" disabled>
                <img src="{{ asset(str_replace('public/', '', $announcement->image_path)) }}" class="w-full h-auto rounded-[15px] max-h-64 object-cover" alt="Current Image">
            </div>
        </div>
